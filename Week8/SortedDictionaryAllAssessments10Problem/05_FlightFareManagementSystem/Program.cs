...
namespace ConsoleApp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                service.AddTicket(new Ticket(parts[0], parts[1], int.Parse(parts[2])));
            }
            catch (Exception ex)
            {
                ...
                try
                {
                    switch (choice)
                    {
                        ...
                        case 2:
                            ...
                            service.UpdateFare(updateId, newFare);
                            break;
                        case 3:
                            string addInput = Console.ReadLine();
                            string[] addParts = addInput.Split(' ');
                            service.AddTicket(new Ticket(addParts[0], addParts[1], int.Parse(addParts[2])));
                            break;
                        case 4:
