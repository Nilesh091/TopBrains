...
namespace UserService.Application.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<bool> RegisterAsync(RegisterDTO dto)
        {
            ...
            if (!created)
                return false;
            // Assign the selected role (Buyer or Seller)
        ...
        public async Task<LoginResponseDTO> LoginAsync(LoginDTO dto, string ipAddress, string userAgent)
        {
            ...
            if (user.IsTwoFactorEnabled)
            {
                ...
                response.UserId = user.Id; // Return UserId so client can use it for OTP verification
                return response;
            }
            ...
            response.Roles = roles.ToList(); // Include roles in response
            response.UserId = user.Id; // Return UserId on successful login
        ...
        public async Task<bool> VerifyConfirmationEmailAsync(ConfirmEmailDTO dto)
        {
            ...
            // 🔥 FIX: Decode token before verification
            var decodedToken = Uri.UnescapeDataString(dto.Token);

            var result = await _userRepository.VerifyConfirmaionEmailAsync(user, decodedToken);
        ...
        public async Task<RefreshTokenResponseDTO> RefreshTokenAsync(RefreshTokenRequestDTO dto, string ipAddress, string userAgent)
        {
            ...
            response.Token = GenerateJwtToken(user, roles, dto.ClientId);
            response.Roles = roles.ToList(); // Include roles in response
            response.UserId = user.Id; // Return UserId on token refresh
        ...
        /// <summary>
        /// Generate a 6-digit OTP and send it to user's email for 2FA during login
        /// </summary>
        public async Task<bool> GenerateAndSendOtpAsync(Guid userId, string email)
        {
            try
            {
                ...
                // Set OTP on user with 10-minute expiry
                user.OtpCode = otp;
        ...
        /// <summary>
        /// Verify OTP provided by user and generate JWT token if valid
        /// </summary>
        public async Task<LoginResponseDTO> VerifyOtpAsync(VerifyOtpDTO dto, string ipAddress, string userAgent)
        {
            var response = new LoginResponseDTO();
            ...
            {
                response.ErrorMessage = "No OTP found. Please login again.";
                return response;
            }
            ...
            if (user.OtpExpiryTime == null || DateTime.UtcNow > user.OtpExpiryTime)
            {
                ...
                user.OtpAttempts = 0;
                await _userRepository.UpdateUserAsync(user);
                response.ErrorMessage = "OTP has expired. Please login again.";
                return response;
            }
            ...
            if (user.OtpAttempts >= 3)
            {
                ...
                user.OtpAttempts = 0;
                await _userRepository.UpdateUserAsync(user);
                response.ErrorMessage = "Maximum OTP attempts exceeded. Please login again.";
                return response;
            }
            ...
            if (user.OtpCode != dto.OtpCode)
            {
                user.OtpAttempts++;
                await _userRepository.UpdateUserAsync(user);
                var attemptsLeft = 3 - user.OtpAttempts;
                response.ErrorMessage = $"Invalid OTP. {attemptsLeft} attempt(s) remaining.";
                response.RemainingAttempts = attemptsLeft;
                return response;
            }
            ...
            response.Token = GenerateJwtToken(user, roles, dto.ClientId);
            response.Roles = roles.ToList(); // Include roles in response
            response.UserId = user.Id; // Return UserId after OTP verification
            response.RefreshToken = await _userRepository.GenerateAndStoreRefreshTokenAsync(user.Id, dto.ClientId, userAgent, ipAddress);
...
new Claim("client_id", clientId)
            ...
            // ✅ FIX: Use Base64 decoding (secret key is Base64-encoded)
